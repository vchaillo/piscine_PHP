...
if ($_GET['total'] && $_SESSION['id'])
    $command_success = create_command($db, mysqli_real_escape_string($db, $_GET['total']));
else if ($_GET['total'])
        ...
        ?>
            <table>
                ...
                foreach ($_SESSION["cart"] as $key => $id) {
                    ...
                    echo '<td>' . $_SESSION["qte"][$key] . '<a href="/minishop/cart.php?action=minus&id=' . $product['id'] . '"><i class="fa fa-minus btn"></i><a href="/minishop/cart.php?action=plus&id=' . $product['id'] . '"><i class="fa fa-plus btn"></i></td>';
                    echo '<td>' . $product['price']*$_SESSION["qte"][$key] . '$<a href="/minishop/cart.php?action=trash&id=' . $product['id'] . '"><i class="fa fa-trash btn"></i>';
                    ?>
                    </tr>
                <?php
                }
            ...
            <div>
                <a href="/minishop/cart.php?total=<?php echo $total ?>"><input type="submit" name="total" value="Pay now" /></a>
            </div>
        <?php
        }
        else
